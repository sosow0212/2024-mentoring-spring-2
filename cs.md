...
---
## 서블릿 필터 vs 스프링 인터셉터
![필터와인터셉터](image/필터와인터셉터.jpg)

### Filters
* Filters의 개요: 필터(Filter)는 웹서버의 일부로, 스프링 프레임워크에 종속되지 않는다.
    * 클라이언트로부터 들어오는 HTTP 요청이나 서버로부터 나가는 HTTP 응답을 가로채어 처리할 수 있음.
    * 필터는 특정 조건에 따라 요청을 차단하거나, 응답을 클라이언트에게 보내지 않도록 막을 수 있음.
### 필터의 활용
* 인코딩, cors, 보안 관련, request 바디 압축.
* 인증 및 인가(authentication and authorization) 작업

### 필터 생성하기
1. javax.servlet.Filter 인터페이스를 구현한 클래스를 작성.
2. doFilter 메서드를 오버라이드하여 요청과 응답을 처리.
...
1. 트랜잭션(Transaction): 트랜잭션은 데이터베이스에서 일련의 작업들이 모두 성공적으로 완료되거나, 하나라도 실패할 경우 전부 취소되는 것을 보장하는 메커니즘.
2. 프록시(Proxy): 프록시는 실제 객체를 감싸서 추가적인 기능을 제공하는 대리 객체. 스프링에서는 AOP(Aspect-Oriented Programming)를 구현하기 위해 프록시를 사용.

### 스프링 AOP와 프록시
* 스프링에서 AOP(Aspect-Oriented Programming)는 주로 프록시 기반으로 작동. AOP를 통해 특정 메서드의 전후에 공통 기능(ex) 로깅, 트랜잭션 관리)을 적용할 수 있다.

1. JDK 동적 프록시: 대상 객체가 인터페이스를 구현하고 있을 때 사용.
2. CGLIB 프록시: 대상 객체가 인터페이스를 구현하지 않고 있을 때 사용.

### 트랜잭션과 프록시의 관계
1. 프록시가 트랜잭션 관리: @Transactional이 적용된 메서드가 호출될 때, 스프링은 프록시 객체를 사용하여 해당 메서드를 가로챔. -> 프록시는 트랜잭션을 시작하고, 메서드를 실행한 뒤 결과에 따라 트랜잭션을 커밋하거나 롤백 진행.
   * ex) Service 클래스의 updateArticle 메서드에 @Transactional이 붙어 있다고 가정 : 클라이언트가 updateArticle을 호출하면, 실제로는 Service의 프록시 객체가 호출. -> 이 프록시는 트랜잭션을 시작한 후, 실제 Service 객체의 updateArticle 메서드를 호출. 메서드 실행이 끝나면 프록시는 트랜잭션을 커밋하거나, 예외가 발생하면 롤백.
2. 프록시 내부 동작의 한계:
   * 자기 호출(self-invocation) 문제: 프록시 객체 내부에서 동일 객체의 다른 메서드를 호출하는 경우, 이 호출은 프록시를 거치지 않고 직접 호출. 이 경우 트랜잭션이 적용되지 않을 수 있음.
   * 해결 방법: 코드 구조를 변경하거나, AopContext.currentProxy()를 사용해 프록시를 참조하여 트랜잭션이 적용되도록 할 수 있음. but 이 방법은 코드가 스프링 AOP에 강하게 결합됌.

### 결론
* 스프링에서 트랜잭션 관리는 주로 프록시를 통해 이루어짐.
* 프록시 객체가 메서드 호출을 가로채고, 트랜잭션을 시작, 커밋, 롤백 진행.
* 프록시는 실제 객체를 감싸서 AOP 기능(예: 트랜잭션)을 함.
* 자기 호출(self-invocation)은 프록시를 우회하기 때문에 트랜잭션이 적용되지 않을 수 있음.
* 즉, 스프링의 트랜잭션 관리는 프록시라는 대리 객체가 뒤에서 마법처럼 일을 처리해주는 방식. 트랜잭션이 필요한 작업이 있으면 프록시가 이를 가로채서 알아서 처리해주는 구조.

---
## AOP
### AOP(Aspect-Oriented Programming) 개념 
* AOP(Aspect-Oriented Programming, 관점 지향 프로그래밍)는 프로그램 실행 시 특정 로직을 모듈화하여 여러 클래스에서 공통적으로 사용될 수 있도록 하는 방법. 

1. Aspect(애스펙트)
   * 정의: 애스펙트는 여러 클래스에 걸쳐 있는 공통적인 관심사를 모듈화한 것.
   * 예시: 트랜잭션 관리, 로깅 등이 대표적인 예.
   * 스프링에서의 구현: 스프링에서는 애스펙트를 일반 클래스로 구현하거나 @Aspect 애너테이션을 사용하여 구현.
2. Join Point(조인 포인트)
   * 정의: 프로그램 실행 중 특정 시점, 예를 들어 메서드 실행이나 예외 처리 등을 의미.
   * 스프링에서의 조인 포인트: 스프링 AOP에서 조인 포인트는 항상 메서드 실행을 나타냄.
3. Advice(어드바이스)
   * 정의: 특정 조인 포인트에서 애스펙트가 취하는 행동.
   * 어드바이스의 종류:
     * Before: 메서드 실행 전에 실행되는 어드바이스.
     * After Returning: 메서드가 정상적으로 완료된 후 실행되는 어드바이스.
     * After Throwing: 메서드가 예외를 던진 후 실행되는 어드바이스.
     * After (Finally): 메서드의 종료 여부와 상관없이 항상 실행되는 어드바이스.
     * Around: 메서드 호출을 감싸서 실행 전후에 실행되며, 메서드 호출을 중단하거나 대체할 수 있는 가장 강력한 어드바이스.
4. Pointcut(포인트컷)
   * 정의: 조인 포인트를 매칭하는 조건(판별자).
   * 스프링에서의 포인트컷: 스프링은 기본적으로 AspectJ의 포인트컷 표현식을 사용. 포인트컷은 특정 메서드 실행에 대해 어드바이스를 실행할 지 여부를 결정.
5. Introduction(인트로덕션)
   * 정의: 기존 타입에 새로운 메서드나 필드를 선언하는 것.
   * 스프링에서의 인트로덕션: 스프링 AOP를 사용해 새로운 인터페이스와 구현을 지정된 객체에 추가할 수 있음.
6. Target Object(대상 객체)
   * 정의: 하나 이상의 애스펙트에 의해 조언을 받는 객체. "Advised Object"라고도 한다.
   * 스프링에서의 대상 객체: 항상 프록시된 객체.
7. AOP Proxy(프록시)
   * 정의: AOP 프레임워크가 애스펙트의 계약(어드바이스 메서드 실행 등)을 구현하기 위해 생성한 객체.
   * 스프링에서의 프록시: JDK 동적 프록시 또는 CGLIB 프록시가 있음.
8. Weaving(위빙)
   * 정의: 애스펙트를 다른 애플리케이션 타입이나 객체와 연결하여 조언된 객체를 생성하는 과정.
   * 스프링에서의 위빙: 런타임에 수행된다.
   * 스프링 AOP에서 제공하는 어드바이스 타입
     * Before Advice: 조인 포인트가 실행되기 전에 실행되는 어드바이스로, 조인 포인트의 실행을 막을 수 없다(예외를 던지는 경우 제외).
     * After Returning Advice: 조인 포인트가 정상적으로 완료된 후 실행.
     * After Throwing Advice: 메서드가 예외를 던지면 실행.
     * After (Finally) Advice: 조인 포인트가 정상적이든 예외적이든 관계없이 항상 실행.
     * Around Advice: 조인 포인트를 감싸는 어드바이스로, 메서드 호출 전후에 실행되며, 메서드 실행을 중단하거나 대체할 수 있음. 가장 강력한 어드바이스 타입.
### 스프링 AOP의 목표와 기능
* 순수 자바로 구현: 스프링 AOP는 순수 자바로 구현되며, 클래스 로더 계층을 제어할 필요가 없어 서블릿 컨테이너나 애플리케이션 서버에서 사용할 수 있음.
* 메서드 실행 조인 포인트만 지원: 스프링 AOP는 스프링 빈의 메서드 실행만을 조언할 수 있음. 필드 접근이나 업데이트 조인 포인트를 조언하려면 AspectJ와 같은 언어를 염두해야 함.
* 스프링 IoC와의 긴밀한 통합: 스프링 AOP는 스프링의 IoC 컨테이너와 긴밀하게 통합되어 있으며, 주로 엔터프라이즈 애플리케이션의 공통 문제를 해결하는 데 초점을 맞추고 있다.
* AspectJ와의 차별점: 스프링 AOP는 세밀한 객체(주로 도메인 객체)를 조언하는 데에는 적합하지 않다. AspectJ는 이러한 경우에 더 적합.
* 프록시 기반 프레임워크: 스프링 AOP는 프록시 기반. 이는 스프링 AOP가 프록시 객체를 통해 AOP 기능을 제공한다는 것을 의미.
* 스프링 AOP에서 프록시 선택
  * JDK 동적 프록시: 기본적으로 인터페이스를 구현한 객체에 대해 사용.
  * CGLIB 프록시: 인터페이스를 구현하지 않은 클래스에 대해 사용.
  * 프록시 선택 기준: 스프링 AOP는 기본적으로 JDK 동적 프록시를 사용하며, 인터페이스를 구현하지 않은 객체에 대해서만 CGLIB을 사용. 필요시 CGLIB을 강제로 사용할 수 있다.
